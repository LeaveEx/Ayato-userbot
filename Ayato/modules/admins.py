...
from Ayato.utils.misc import extract_user, extract_user_and_reason, list_admins
...
async def set_chat_photo(client: Client, message: Message):
    ...
    if not (can_change_admin or can_change_member):
        await message.edit_text("Anda tidak memiliki cukup izin")
    if message.reply_to_message:
        ...
        await message.edit_text("Balas foto untuk menyetelnya!")
...
async def pin_message(client: Client, message):
    if not message.reply_to_message:
        return await edit_or_reply(message, "Balas pesan untuk menyematkan/melepas sematnya.")
    Man = await edit_or_reply(message, "`Processing...`")
    bot = (await client.get_chat_member(message.chat.id, client.me.id)).privileges
...
async def demote(client: Client, message: Message):
    ...
    if user_id == client.me.id:
        return await Man.edit("Aku tidak bisa menurunkan diriku sendiri.")
    await message.chat.promote_member(
        user_id,
        privileges=ChatPrivileges(
