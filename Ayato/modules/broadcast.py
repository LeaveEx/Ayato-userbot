...
#from Ayato.helpers.misc import HAPP, in_heroku
from Ayato.helpers.tools import get_arg
from Ayato.utils.misc import restart
...
while 0 < 6:
    ...
    if _GCAST_BLACKLIST.status_code != 200:
        if 0 != 5:
            continue
        GCAST_BLACKLIST = [-1001812143750, -1001473548283, -1001390552926, -1001302879778, -1001600598969]
        break
...
async def gcast_cmd(client: Client, message: Message):
    if message.reply_to_message or get_arg(message):
        Man = await edit_or_reply(message, "`SABAR KONTOLL INI GW MAU NYEBARIN KATA KATA JAMET LU...`")
    else:
        return await message.edit_text("**LU NGEPRANK YA MEMEK**")
    done = 0
    error = 0
...
async def gucast_cmd(client: Client, message: Message):
    if message.reply_to_message or get_arg(message):
        ...
        return await message.edit_text("**LU NGEPRANK GW YA MEMEK**")
    done = 0
    error = 0
    async for dialog in client.get_dialogs():
