...
from Ayato.utils import extract_user, extract_user_and_reason
...
def globals_init():
    try:
        ...
        sql = import_module("Ayato.helpers.SQL.gban_sql")
        sql2 = import_module("Ayato.helpers.SQL.gmute_sql")
...
async def gban_user(client: Client, message: Message):
    ...
    if not user_id:
        return await Man.edit("KIRIM PENGGUNA YANG BENER KONTOLLL.")
    if user_id == client.me.id:
        return await Man.edit("**LU MAU BAN DIRI SENDIRI MENDING HAPUS AKUN**")
    if user_id in DEVS:
        return await Man.edit("**NGAPAIN GBAN BAGINDA GW KONTOLL**")
    if user_id in WHITELIST:
        return await Man.edit(
            "**LU GABISA GBAN DIA KARNA DIA ADMIN TOLOL**"
        )
    if user_id:
        try:
    ...
    if not f_chats:
        return await Man.edit("**Anda tidak mempunyai GC yang anda admin 🐻‍❄️**")
    er = 0
    done = 0
...
async def gmute_user(client: Client, message: Message):
    ...
    if user.id == client.me.id:
        return await Man.edit("**KALAU MAU DI MUTE LU MUTE KONTOL LU SENDIRI**")
    if user.id in DEVS:
        return await Man.edit("**NGAPAIN MAU MUTE BAGINDA GW KONTOL**")
    if user.id in WHITELIST:
        return await Man.edit(
            "**LU GABISA GMUTE DIA KARNA DIA TUHAN GW YA MEMEK**"
        )
    try:
...
async def gmutelist(client: Client, message: Message):
    ...
    Man = await edit_or_reply(message, "`Processing...`")
    if not users:
        return await Man.edit("BELUM ADA ANAK KAMPANG YANG DI MUTE")
...
add_command_help(
    ...
    [
        ...
        ["ungmute <reply/username/userid>", "Membatalkan Global mute."],
