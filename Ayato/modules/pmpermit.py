...
from Ayato import TEMP_SETTINGS
from Ayato.helpers.adminHelpers import DEVS
from Ayato.helpers.basic import edit_or_reply
from Ayato.helpers.SQL.globals import addgvar, gvarstatus
from Ayato.helpers.tools import get_arg
...
DEF_UNAPPROVED_MSG = (
    ...
    f"- 🐻‍❄️Babu Nya `{DEFAULTUSER}`\n"
    "- DIKENTOT OLEH 🐻‍❄️Kampang-BOT🐻‍❄️")
)
...
async def incomingpm(client: Client, message: Message):
    try:
        ...
        from Ayato.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from Ayato.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
    ...
    if message.chat.id in DEVS:
        try:
            ...
            await client.send_message(
                message.chat.id,
                f"<b>Menerima Pesan!!!</b>\n{message.from_user.mention} <b>tTERDETEKSI ANAK KAMPANG</b>",
...
async def approvepm(client: Client, message: Message):
    try:
        from Ayato.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from Ayato.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from Ayato.helpers.SQL.globals import addgvar
...
async def setpmpermit(client: Client, cust_msg: Message):
    ...
    try:
        import Ayato.helpers.SQL.globals as sql
    except AttributeError:
        await cust_msg.edit("**Running on Non-SQL mode!**")
        return
    Man = await cust_msg.edit("`Processing...`")
