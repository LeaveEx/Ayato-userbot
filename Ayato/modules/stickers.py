...
from Ayato.helpers.tools import get_arg, get_text, resize_media
from Ayato.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            return
        media_ = await client.download_media(replied, file_name="Ayato/resources/")
    else:
        ...
        if exist is not False:
            ...
            while limit in await get_response(message, client):
                ...
                if await get_response(message, client) == "Invalid pack selected.":
                    ...
                    await asyncio.sleep(2)
                    await Man.edit(
                        f"**Sticker Berhasil Ditambahkan!**\n         🐻‍❄️ **[KLIK DISINI](https://t.me/addstickers/{packname})** 🐻‍❄️\n**Untuk Menggunakan Stickers**"
                    )
                    return
            await client.send_document("stickers", media_)
            await asyncio.sleep(2)
...
async def tinying(client: Client, message: Message):
    ...
    im1 = Image.open("Ayato/resources/blank.png")
    if ik.endswith(".tgs"):
